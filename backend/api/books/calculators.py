...
from core.utils.intervals import TimeInterval
        ...
        for line in self.sale.lines.all():
            ...
            if self.tax_rate is None:
                ...
                raise NotImplementedError(
                    "the system doesn't support " "yet multiple tax rates " "into a same invoice"
                )
...
class ProfitsLossCalculator(object):
    ...
    def __init__(self, contract, sum_type=SUM_TYPE_COLLECTED, start=None, end=None):
    ...
    def process_generator(self, sales_queryset):
        ...
        So it yield a tuple similar to
            (sale, payment, amount)
        """
        sales_queryset = sales_queryset.filter(contract=self.contract)
        ...
        for sale in sales_queryset:
            for pay in sale.payments.all():
                ...
                #     `self.payments.all()` is uncorrelated with the filters
    ...
    def total_collected(self):
        collected = D("0")
        invoices_queryset = self.contract.invoices.all()
    ...
    def total_expenses(self):
        expenses = D("0")
        bills_queryset = self.contract.bills.all()
