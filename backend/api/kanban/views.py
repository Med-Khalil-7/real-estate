...
class BoardViewSet(
    ...
    def get_queryset(self):
        ...
        if self.action == "retrieve":
            ...
            if assignees:
                queryset = (
                    Task.objects.filter(Q(assignees__in=[int(x) for x in assignees.split(",")]))
                    .order_by("id")
                    .distinct("id")
                )
    ...
    def get_member(self):
        try:
            member = User.objects.get(email=self.request.data.get("email"))
    ...
    def invite_member(self, request, pk):
        ...
        return Response(data=UserSerializer(instance=new_members, many=True).data)
    ...
    def remove_member(self, request, pk):
        ...
        return Response(data=UserSerializer(instance=member).data)
...
class CommentViewSet(
    ...
    def get_queryset(self):
        return super().get_queryset().filter(task__column__board__members=self.request.user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if (
            self.request.user
            not in Task.objects.get(id=request.data.get("task")).column.board.members.all()
        ):
            return Response(status=HTTP_400_BAD_REQUEST)
...
class SortTask(APIView):
    ...
    def move_tasks(self, request):
        ...
        pre_tasks = Task.objects.filter(column__in=pre_columns).prefetch_related("columns")
...
def sort_model(request, Model):
    ...
    objects_dict = dict([(str(obj.pk), obj) for obj in Model.objects.filter(pk__in=ordered_pks)])
    order_field_name = Model._meta.ordering[0]

    step = 1
    start_object = min(objects_dict.values(), key=lambda x: getattr(x, order_field_name))
