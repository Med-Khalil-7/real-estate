...
import api from "../../../api";
import {EXPENSE_CLAIM, PROPERTY_CONTRACTS, TAX_RATE} from "../../../constants/api";
...
const ClaimPage = ({data, pdfMode}) => {
  ...
  const {contractId, claimId} = useParams() // route args
  const [errors, setErrors] = useState({});
  const history = useHistory()
  const {permissions} = usePermission()
  const canEdit = permissions.includes("core.change_expenseclaim")
  ...
  const fetchClaim = async () => {
    try {
      const {data} = await api.get(`${EXPENSE_CLAIM}${claimId}`)
      setInvoice(data)
    } catch (e) {
      toast.error("Error fetching expense claim")
    }
  }
  ...
  const handleSubmit = async () => {
    try {
      if (!claimId) {
        await api.post(`${EXPENSE_CLAIM}`, invoice)
        toast.success("Expense claim added successfully")
        history.goBack()
      } else {
        await api.put(`${EXPENSE_CLAIM}${claimId}/`, invoice)
