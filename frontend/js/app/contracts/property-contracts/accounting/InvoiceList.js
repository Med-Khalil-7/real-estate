...
import api from "../../../../api";
import {INVOICE, PAY_INVOICE} from "../../../../constants/api";
import DataTable from "react-data-table-component";
import usePermission from "../../../hooks/usePermission";


function InvoiceList({contractId}) {
      ...
      cell: row => {
        return (
          <div className="d-flex justify-content-between">
            {is_tenant | is_landlord | permissions.includes("core.view_invoice") && (<Link
              to={`/accounting/invoice/add/${contractId}/${row.id}/`}
            >
            ...
            {
              row.is_fully_paid ? (
                  <Button disabled variant="btn btn-success" size="sm">
                    {t("Payed")}
                  </Button>) :
                (<Button
                  disabled={!permissions.includes("core.add_payment")}
                  onClick={() => {
                    setInvoiceToPay(row.id);
                    handleShowPayment()
  ...
  const payInvoice = async () => {
    try {
      await api.post(
        PAY_INVOICE.replace("{invoice_id}", invoiceToPay)
      );
  ...
  return (
    <>
      <Modal show={showPayment} onHide={handleClosePayment}>
        ...
        <Modal.Body>
          <Modal.Body>
            {t("Are you sure you want to create a payment for this invoice?")}
          </Modal.Body>
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => payInvoice()}>
            {t("Pay")}
          </Button>
        </Modal.Footer>
      </Modal>
        ...
        <div className="card">
          <div className="card-body">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div>
                {is_superuser && <Button as={Link} to={`/accounting/invoice/add/${contractId}/`}
