...
import api from '../../api';
import { DISCOUNTS } from '../../constants/api';
...
export default function List() {
  ...
  const handleDelete = (row) => {
    setDiscountToRemove(row);
    handleShow();
  };

  const deleteDiscount = async () => {
    try {
      await api.delete(`${DISCOUNTS}${discountToRemove}/`);
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>{t('Discount removal')}</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={deleteDiscount}>
            {t('Delete')}
          </Button>
        </Modal.Footer>
      </Modal>
        ...
        <Card.Body>
          <Card.Title>
            ...
            {/*       <div className="d-flex align-items-end flex-column">
              <Link className="mt-auto p-2" to="/discount/add">
                <Button variant="primary">{t('Add new discount')} </Button>
              </Link>
            </div> */}
          </Card.Title>
