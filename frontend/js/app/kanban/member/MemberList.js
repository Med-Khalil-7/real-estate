...
import { selectAllMembers, setMemberListOpen, setDialogMember } from './MemberSlice';
...
const MemberListDialog = () => {
  ...
  const open = useSelector((state) => state.member.memberListOpen);
  const [searchValue, setSearchValue] = useState('');
  const xsDown = useMediaQuery(theme.breakpoints.down('xs'));

  const filteredMembers = members.filter((member) =>
    member.email.toLowerCase().match(searchValue.trim().toLowerCase())
  );
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth fullScreen={xsDown}>
      <Container>
