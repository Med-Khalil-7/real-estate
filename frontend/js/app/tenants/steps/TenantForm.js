...
export default function TenantForm({
  ...
  const checkValidation = async () => {
    const result = await trigger(['fist_name', 'last_name', 'email', 'gender', 'password']);
    if (!result) return;
    nextStep();
  };
    ...
    <>
      ...
      <div className="form-row">
        ...
        <div className="form-group col-md-6">
          ...
          />
          <div className="invalid-feedback">{errors.user?.confirmPassword?.message}</div>
        </div>
      </div>
      <div className="form-row">
        ...
        <div className="form-group col-md-4">
          ...
          <input
            ...
            className={`form-control ${errors?.user?.address?.district ? 'is-invalid' : ''}`}
          />
          <div className="invalid-feedback">{errors?.user?.address?.district?.message}</div>
        </div>
      </div>
      {!isBusy && (
        <div className="form-row">
